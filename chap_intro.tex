...
\end{equation}	
In geophysics we are interested in defining the physical properties  of the subsurface (e.g. seismic velocity, density) from experimental data, $\bm{y}$, observed at the surface (e.g. seismic signals, gravitational acceleration). By using physical models, $\mathcal{M}$, which link a parameterized subsurface and resulting simulated data, $\bm{y'}$, the pursuit of imaging the subsurface collapses to a parameter inference problem \citep{Tarantola2005}.\par

Geophysical parameter inference problems are commonly refered to as \textit{inverse problems} \citep{Tarantola2005,Aster2013,Menke2012}. They start with the experimental data, $\bm{y}$, and try to infer the causative model parameters, $\bm{\theta}$. As opposed to \textit{forward problems}, which start with a given set of model parameters, $\bm{\theta'}$, and produce a set of simulated data, $\bm{y'}$. Both areas are subject to considerable research and development. The ability to solve the forward problem is required to solve the inverse problem.
\begin{equation}
...
The prior, $p(\bm{\theta})$, is a user-defined probability distribution which incorporates what is already known about the model parameters, $\bm{\theta}$, independent of the experimental data, $\bm{y}$. For example, ...\par
...
For non-trivial problems of scientific interest there is no analytical solution for the posterior. One of the possible methods to then define the posterior $p(\bm{\theta}|\bm{y})$ is stochastic sampling such as Monte Carlo sampling or Markov chain Monte Carlo (MCMC) sampling. For problems where there are many unknown model parameters and each sample is computationally expensive, highly efficient sampling methods are needed for the posterior to be computationally tractable. \par
...
A full derivation of the likelihood is featured in \hyperref[tf1]{Technical figure 1} and \hyperref[tf1]{Technical figure 2}, from first assumptions (\hyperref[tf1]{Technical figure 1}) to the final forms which are commonly featured in geophysical inversions (\hyperref[tf1]{Technical figure 2}).\\
