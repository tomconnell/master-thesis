...
\begin{figure}[H]
	\centering
	\includegraphics[scale=1.1]{linear-regression-example}
...
\end{bmatrix} $ where both $\bm{\mu}$ and $\bm{\sigma}$ are unknown, $\bm{\theta} = [\bm{\mu},\bm{\Sigma}]$. Figure XX shows the observed data and the underlying causitive distribution. This example has 5 unknown parameters in total and as such we can appeal to ABC-MCMC for efficient sampling. The weighting kernel takes the same for as equation \ref{weight-kernel} and summaries with a 1:1 correspondance to the unknown parameters are used, the marginal sample means and standard deviations, as well as sample covariance. Figure YY shows the results of the first 1000 time steps from an ABC-MCMC algorithm form described in section \ref{sec-lin-reg}. It can be seen that the algorithm is stationary for the first X repititions.\\
...
\section{Toy problem 4: Banana distribution}
 ...
 \begin{figure}[H]
 	\centering
 	\includegraphics[scale=0.75]{ABCvsMCMC}
 	\caption{}
 \end{figure}